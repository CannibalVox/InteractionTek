...
public class ActiveHotbarMatcher extends Slot {
    ...
    public boolean test(Ref<EntityStore> user, CommandBuffer<EntityStore> commandBuffer, InteractionContext context, ItemContainer targetContainer, int targetSlot, ItemStack targetItem) {
        InventoryComponent.Hotbar hotbar = commandBuffer.getComponent(user, InventoryComponent.Hotbar.getComponentType());
        if (hotbar == null) {
            return false;
        }

        return (targetContainer == hotbar.getInventory() && targetSlot == hotbar.getActiveSlot());
    }
}
